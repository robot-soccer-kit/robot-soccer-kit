...
void motors_set_ik(float dx, float dy, float dt) {
  float w1 = MOTORS_SIGN * (motors[0].drive_x * dx + motors[0].drive_y * dy +
              MODEL_ROBOT_RADIUS * dt) /
             (MODEL_WHEEL_RADIUS);
  float w2 = MOTORS_SIGN * (motors[1].drive_x * dx + motors[1].drive_y * dy +
              MODEL_ROBOT_RADIUS * dt) /
             (MODEL_WHEEL_RADIUS);
  float w3 = MOTORS_SIGN * (motors[2].drive_x * dx + motors[2].drive_y * dy +
              MODEL_ROBOT_RADIUS * dt) /
             (MODEL_WHEEL_RADIUS);
