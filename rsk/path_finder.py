...
class PathFinder(astar.AStar):
    ...
    def angles(self) -> list[float]:
        ...
        """
        return [
            k * (2 * np.pi / self.discretization) for k in range(self.discretization)
        ]
    ...
    def add_obstacle(self, x: float, y: float, radius: float):
        ...
        for k in self.angles():
            # Storing all nodes
            self.add_node(
                x + (radius + self.avoid_margin) * np.cos(k),
                y + (radius + self.avoid_margin) * np.sin(k),
            )
    ...
    def find_target(
        self, start: int, goal: int, target_distance: float = 0.25
    ) -> np.ndarray:
        """
